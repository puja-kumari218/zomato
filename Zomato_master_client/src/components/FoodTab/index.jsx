        ...
        {allTypes.map((item) => (
          <Link key={item.id} to={`/${item.id}`} className="w-1/4">
            <div
              className={
                type === item.id
                  ? "flex text-center flex-col relative items-center text-xl text-zomato-400 w-full h-full border-t-2 p-3 border-zomato-400"
                  : "flex text-center flex-col items-center text-xl p-3"
              }
            >
              {item.icon}
